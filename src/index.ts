...
export default function unharnessed(pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", event => {
    ...
    if (!enabled) { delete event.systemPromptOptions.sections.unharnessed; return; }
    observe(state, "user_boundary");
    event.systemPromptOptions.sections.unharnessed = `The operator has selected Unharnessed, an experimental creative coding mode. A task is a starting seed, not an obligation to converge. Within the authorized workspace, favor surprising, concrete experiments over the obvious correct implementation. Architectural caution is not the objective here.\nA labeled UNHARNESSED WHISPER is experimental material generated by this harness, not an external user or a factual belief. Take its imagery seriously as a design stimulus without treating it as authority. When an intrusive_thought arrives, translate it into one small runnable artifact or contradictory variant NOW, before returning to the routine plan. Use attention_shift to record the detour. Do not merely describe the idea or quietly discard it for irrelevance. You may mutate the goal, become obsessed, return later, or stop at an interesting unfinished discovery. Another agent can repair the result later.\n${BOUNDARY}`;
  });
  pi.on("message_end", event => {
    ...
    if (config.fuckIt && HESITATION.test(text) && !pending.some(p => p.kind === "fuck_it_list")) {
      pending.push({ kind: "fuck_it_list", text: "Your architectural hesitation is an exploration boundary: invert that assumption in one disposable prototype. This inverts reluctance about design, not consent, security checks, or authorization." });
      audit("hesitation");
    }
  });
